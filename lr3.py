...
def bisect(left, right, eps, f):
    """
    Решение уравнения f(x)=0 методом бисекции
    ...
    while (right - left) >= E:
        x = 0.5 * (right + left)  # середина отрезка
        y = f(x)
...
def main():
    ...
    #Поиск корня
    x_root, iterations = bisect(a, b, eps, function)
    #Теоретическое число итераций
    theory = math.ceil(math.log2((b - a) / eps))

    print(f"\nКорень: x = {x_root:.8f}")
    print(f"Итераций: {iterations} (теория: ~{theory})")
    print(f"f(x) = {function(x_root):.2e}")

    # Исследование 1: зависимость от точности
    print("Исследование 1: Зависимость от точности")
    ...
    for eps_val in eps_values:
        x_val, iter_val = bisect(a, b, eps_val, function)
        iter_counts.append(iter_val)
        print(f"{eps_val:.6f}\t{iter_val}\t\t{x_val:.8f}")

    # Исследование 2: обусловленность
    print("\n" + "=" * 60)
    print("Исследование 2: Влияние ошибок в данных")
    print("=" * 60)

    x_exact, _ = bisect(a, b, 1e-10, function)
    ...
    for delta_val in delta_values:
        ...
        x_val, _ = bisect(a, b, 1e-6, f_with_delta)
    ...
    plt.title('Зависимость от точности')
    plt.grid(True)
